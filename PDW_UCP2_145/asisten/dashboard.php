...
// [PERUBAHAN] Mengganti include header ke file yang sudah di-tema
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    <div class="bg-white p-6 rounded-lg shadow-md flex items-center space-x-4">
        ...
        <div>
            ...
            <p class="text-2xl font-bold text-slate-800"><?php echo $total_modul; ?></p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md flex items-center space-x-4">
        ...
        <div>
            ...
            <p class="text-2xl font-bold text-slate-800"><?php echo $total_laporan; ?></p>
        </div>
    </div>

    <div class="bg-white p-6 rounded-lg shadow-md flex items-center space-x-4">
        <div class="bg-amber-100 p-3 rounded-full">
            <svg class="w-6 h-6 text-amber-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        </div>
        <div>
            <p class="text-sm text-gray-500">Laporan Belum Dinilai</p>
            <p class="text-2xl font-bold text-slate-800"><?php echo $laporan_pending; ?></p>
        </div>
    </div>
</div>
        ...
        <?php if ($recent_activities && $recent_activities->num_rows > 0): ?>
            <?php while($activity = $recent_activities->fetch_assoc()): ?>
                <div class="flex items-center p-2 hover:bg-gray-50 rounded-lg">
                    <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center mr-4 shrink-0 border border-blue-200">
                        <span class="font-bold text-blue-700 text-sm"><?php echo getInitials($activity['nama_mahasiswa']); ?></span>
                    </div>
                    <div class="flex-grow">
                        <p class="text-gray-700">
                            <strong class="font-semibold text-blue-700"><?php echo htmlspecialchars($activity['nama_mahasiswa']); ?></strong>
                            mengumpulkan laporan untuk
                            <strong class="font-semibold text-slate-700"><?php echo htmlspecialchars($activity['nama_modul']); ?></strong>
                        </p>
